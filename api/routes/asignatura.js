...
router.get('/:nombre', (req, res) => {
    asignatura.find({$or: [{"_id": req.params.nombre} , {$text: {$search: req.params.nombre}} ]})
        .exec()
        .then( x => res.status(200).send(x) );
});
*/
...
router.get('/nombre/:texto', (req, res) => {
    asignatura.find({$text: {$search: req.params.texto}})
        .exec()
        .then( x => res.status(200).send(x) );
});

router.get('/:id', (req, res) => {
    asignatura.findById(req.params.id)
        .exec()
        .then( x => res.status(200).send(x) )
});
...
router.post('/', (req, res) => {
    asignatura.create(req.body).then( x => res.status(201).send(x))
});

router.put('/:id', (req, res) => {
    asignatura.findByIdAndUpdate(req.params.id, req.body)
        .then(() => res.sendStatus(204))
});

router.delete('/:id', (req, res) => {
    asignatura.findByIdAndDelete(req.params.id).exec().then(() => res.sendStatus(204))
});
